...
function inject_mbd() {
  ...
  if (buttonHolder) {
    ...
    mbd_action.onclick = async () => {
      ...
      let username = anchor_tags[8]?.textContent  || 'Yassine Landa';
      let profile_url = anchor_tags[8]?.href || '';
      let img = anchor_tags[7]?.getElementsByTagName('img')[0]?.src || '';
      let user_id = anchor_tags[8]?.href?.split('/')[3]?.split('@')[1]?.split('?')[0] || 'yassine_landa'
