...
function inject_mbd() {
  ...
  for (let i = 0; i < len; ++i) {
      ...
      let published_at = feed[i]?.getElementsByClassName("update-components-actor__sub-description")[0]?.textContent?.split(' ')[0]
      ...
      if(feed[i]?.getElementsByClassName("social-details-social-counts__reactions-count").length){
        like_count  = feed[i]?.getElementsByClassName("social-details-social-counts__reactions-count")[0]?.textContent
      }
      //let comment_count = comment_counts[i]?.innerText.split(' ')[0]
      let comment_count = feed[i]?.getElementsByClassName("social-details-social-counts__comments")[0]?.textContent?.replace(/\n/g, '')?.trim()?.split(' ')[0]
      let repost_count = feed[i]?.getElementsByClassName("social-details-social-counts__item--with-social-proof")[1]?.getElementsByTagName("span")[0]?.textContent?.replace(/\n/g, '')?.trim()?.split(' ')[0]
